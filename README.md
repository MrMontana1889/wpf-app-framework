...
- **Intent over implementation** — Names and structure encode _what_ the system is, not _how_ it is currently implemented
...
This framework is consumed as **source** via direct project references. It is **not currently packaged** as a NuGet dependency.

#### Repository Setup

The framework no longer uses Git submodules.

Instead, both repositories must be cloned **side-by-side** (parallel) on disk.
...
```shell
git clone https://github.com/MrMontana1889/wpf-app-framework.git
git clone https://github.com/PondPackMan2023/PropertyTools.git
```

#### Project References

`wpf-app-framework` references `PropertyTools` projects directly via relative paths:

```xml
..\PropertyTools\<project>\<project>.csproj
```

Because of this, the folder layout **must remain consistent**.

> ***_NOTE:_*** The PropertyTools fork at https://github.com/PondPackMan2023/PropertyTools should be used. 
> Ensure that `PropertyTools.Wpf` supports `net10.0-windows`, as required by this framework.

### Consumers
 
Known consumers:

- _BentleyBuildApp.Next_
- _mission-personal-ledger_ 
